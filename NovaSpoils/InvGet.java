package nova.novaprison;

import net.novaprison.spoils.Spoils;
...
public class InvGet {
    ...
    public static Inventory getInventory() {
        ...
        for (int x = 0; x < 54; x++) {
            String line = Spoils.getInstance().getConfig().getString("Items." + x);
            if (line != null) {
                data.add(line);
            } else {
                data.add("null");
    ...
    public static void saveItems(Inventory inventory) throws IOException {
        ...
        for (int x = 0; x < ser.length; x++) {
            if (!ser[x].equalsIgnoreCase("null")) {
                Spoils.getInstance().getConfig().set("Items." + x, ser[x]);
            } else {
                Spoils.getInstance().getConfig().set("Items." + x, null);
            }
        }
        Spoils.getInstance().saveConfig();
    }
}